...
function groupMoviesDecades(movies) {
  const starWarsDecades = {}
  for (const movie of movies) {
    const decade = Math.floor(movie.releaseYear / 10) * 10
    if (!starWarsDecades[decade]) {
      starWarsDecades[decade] = []
    }
    starWarsDecades[decade].push(movie.title)
  }
  return starWarsDecades
}

const resultado = groupMoviesDecades(starWarsMovies)
